...
OpsBoard 是一個模擬「服務監控 + incident 應變」場景的微服務 demo 系統，多服務協作、API Gateway 路由、關聯式資料庫與快取的實際應用場景（不是裝飾用），以及前後端整合的完整流程。系統模擬一個簡化版的 on-call 工作流程：監控 3 個服務的健康狀態，當其中一個服務異常時觸發告警建立 incident，並可以推進 incident 的處理狀態、查看處理歷程與對應的應變手冊。
